...
namespace WopiCore.Models
{
    ...
    {
        internal UnlockAndRelockRequest(HttpRequest httpRequest, string fileId) : base(httpRequest, fileId)
        {
            Lock = GetHttpRequestHeader(httpRequest, WopiResponseHeaders.LOCK);
            OldLock = GetHttpRequestHeader(httpRequest, WopiRequestHeaders.OLD_LOCK);
        }
